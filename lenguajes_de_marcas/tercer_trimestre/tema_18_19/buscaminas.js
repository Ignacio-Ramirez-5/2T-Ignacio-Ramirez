...
function marcar(miEvento){
    if (miEvento.type === "contextmenu"){ 
        ...
        //detenemos el burbujeo del evento y su accion por defecto miEvento.stopPropagation(); 
...
function generarCampoMinasVacio(){ 
    ...
    buscaminas.aCampoMinas = new Array(buscaminas.numFilas); for (let fila=0; fila<buscaminas.numFilas; fila++){ 
...
function contarMinas(){ 
    ...
    for (let fila=0; fila<buscaminas.numFilas; fila++){ 
        for (let columna=0; columna<buscaminas.numColumnas; columna++){ //solo contamos si es distinto de bomba 
            if (buscaminas.aCampoMinas[fila][columna]!="B"){ 
            ...
            if (!casilla.classList.contains("destapado")){ 
                ...
                if (!casilla.classList.contains("icon-bandera")){ 
                    ...
                    //si no es bomba 
                        ...
                        }else if (buscaminas.aCampoMinas[fila][columna]=="B"){ // si por el contrario hay bomba quitamos la B 
                            casilla.innerHTML = ""; 
...
function actualizarNumMinasRestantes(){ 
    document.querySelector("#numMinasRestantes").innerHTML = (buscaminas.minasTotales - buscaminas.numMinasEncontradas); }
